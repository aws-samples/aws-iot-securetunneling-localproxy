...
namespace iot {
    namespace securedtunneling {
        namespace test {
            ...
            LocalproxyConfig get_localproxy_config(
                const std::uint16_t &port, const string &auth = ""
            ) {
            ...
            struct TestContext {
                ...
                TestContext(const string &auth = "")
                    ...
                    , local_address(
                          http_server.listener->acceptor_.local_endpoint()
                      )
                    , log {}
                    ...
                    , ec {} {
                    cout << "Test HTTP server is listening on address: "
                ...
                void start() {
                    ...
                    http_server_thread
                        = make_unique<thread>([this]() { http_server.run(); });
                    https_proxy_adapter_thread
                        = make_unique<thread>([this, on_tcp_tunnel]() {
                              https_proxy_adapter.async_connect(
                                  on_tcp_tunnel, wss, local_address
                              );
                              ioc.run();
                          });
                }
            ...
            TEST_CASE("Unit tests for WebProxyAdapter.h-happy-case") {
                cout << "Test HTTPS proxy adapter base case with no credentials"
                     << endl;
                ...
                REQUIRE(
                    test_context.ec.message()
                    == WebProxyAdapterErrc_category().message(
                        (int) WebProxyAdapterErrc::Success
                    )
                );
                REQUIRE(
                    static_cast<WebProxyAdapterErrc>(test_context.ec.value())
                    == WebProxyAdapterErrc::Success
                );
                test_context.stop();
            }

            TEST_CASE("Unit tests for WebProxyAdapter.h-with credentials") {
                cout << "Test HTTPS proxy adapter handling of valid "
            ...
            TEST_CASE(
                "Unit tests for WebProxyAdapter.h-bad credentials credentials"
            ) {
                cout << "Test HTTPS proxy adapter handling of bad credentials "
                        "response"
                     << endl;
                ...
                REQUIRE(
                    test_context.ec.message()
                    == WebProxyAdapterErrc_category().message(
                        (int) WebProxyAdapterErrc::ClientError
                    )
                );
                REQUIRE(
                    static_cast<WebProxyAdapterErrc>(test_context.ec.value())
                    == WebProxyAdapterErrc::ClientError
                );
                test_context.stop();
            }

            TEST_CASE("Unit tests for WebProxyAdapter.h-500 error") {
                cout << "Test HTTPS proxy adapter handling of 500 response"
                     << endl;
                ...
                REQUIRE(
                    test_context.ec.message()
                    == WebProxyAdapterErrc_category().message(
                        (int) WebProxyAdapterErrc::ServerError
                    )
                );
                REQUIRE(
                    static_cast<WebProxyAdapterErrc>(test_context.ec.value())
                    == WebProxyAdapterErrc::ServerError
                );
                test_context.stop();
            }

            TEST_CASE("Unit tests for WebProxyAdapter.h-100 error") {
                cout << "Test HTTPS proxy adapter handling of 100 response"
                     << endl;
                ...
                REQUIRE(
                    test_context.ec.message()
                    == WebProxyAdapterErrc_category().message(
                        (int) WebProxyAdapterErrc::OtherHttpError
                    )
                );
                REQUIRE(
                    static_cast<WebProxyAdapterErrc>(test_context.ec.value())
                    == WebProxyAdapterErrc::OtherHttpError
                );
                test_context.stop();
            }

            TEST_CASE("Unit tests for WebProxyAdapter.h-300 error") {
                cout << "Test HTTPS proxy adapter handling of 300 response"
                     << endl;
                ...
                REQUIRE(
                    test_context.ec.message()
                    == WebProxyAdapterErrc_category().message(
                        (int) WebProxyAdapterErrc::RedirectionError
                    )
                );
                REQUIRE(
                    static_cast<WebProxyAdapterErrc>(test_context.ec.value())
                    == WebProxyAdapterErrc::RedirectionError
                );
                test_context.stop();
            }
        }
