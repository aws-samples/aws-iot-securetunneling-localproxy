    ...
    namespace securedtunneling {
        namespace test {
            TEST_CASE("Unit tests for Url.h-happy-cases") {
                ...
                const vector<string> authNs { "",
                                              "user:12345",
                                              "user:F%40o%3Ao%21B%23ar+%24",
                                              "user:password",
                                              "%40fasdbla:12345" };
                const vector<string> authNs_decoded { "",
                                                      "user:12345",
                                                      "user:F@o:o!B#ar $",
                                                      "user:password",
                                                      "@fasdbla:12345" };
                ...
                for (int i = 0; i < n; ++i) {
                    const string https_proxy = protocols.at(i) + "://"
                        + (authNs.at(i).empty() ? "" : authNs.at(i) + "@")
            ...
            TEST_CASE("Unit tests for url_decode-happy case") {
                ...
                const map<string, string> url_encoded_lookup {
                    ...
                    { "%40fasdbla", "@fasdbla" },
                    { "%24%20%26%20%3C%20%3E%20%3F%20%3B%20%23%20%3A%20%3D%20%"
                      "2C%20%22%20%27%20~%20%2B%20%25",
                      "$ & < > ? ; # : = , \" ' ~ + %" }
                };
