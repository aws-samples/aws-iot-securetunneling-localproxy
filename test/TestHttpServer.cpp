...
void handle_request(
    ...
    const auto client_error = [&req](std::string why, http::status status) {
    ...
    if (req.method() != http::verb::connect)
        return send(
            client_error("Unknown HTTP-method", http::status::bad_request)
        );

    // We will use the auth information to indicate to the server how respond so
    // that we can test how the HTTPS Proxy adapter will handle different
    // scenarios
    ...
    if (!encoded_auth.empty()) {
        ...
        const auto result = base64::decode(
            &incoming_auth[0],
            encoded_auth.substr(6).data(),
            encoded_auth.length() - 6
        );
        incoming_auth.resize(result.first);
        string allowed_auth = aws::iot::securedtunneling::test::username + ":"
            + aws::iot::securedtunneling::test::password;
        if (!incoming_auth.empty() && incoming_auth == "500")
            return send(server_error("Server failure"));

        if (!incoming_auth.empty() && incoming_auth == "300")
            return send(
                client_error("REDIRECT", http::status::permanent_redirect)
            );
    ...
    namespace securedtunneling {
        namespace test {
            ...
            TestHttpServer::TestHttpServer(
                const string &address, const unsigned short port
            )
                : port(port) {
                this->address = boost::asio::ip::make_address(address);
                this->listener = std::make_shared<Listener>(
                    ioc, tcp::endpoint { this->address, port }
                );
                this->listener->run();
            }
            ...
            void Session::send_lambda::operator()(
                ...
                auto sp
                    = std::make_shared<http::message<isRequest, Body, Fields>>(
                        std::move(msg)
                    );

                // Store a type-erased version of the shared pointer in the
                // class to keep it alive.
                self_.res_ = sp;
            ...
            void Session::do_read() {
                // Make the request empty before reading, otherwise the
                // operation behavior is undefined.
                req_ = {};
            ...
            void Session::on_write(
                error_code ec, std::size_t bytes_transferred, bool close
            ) {
                boost::ignore_unused(bytes_transferred);
            ...
            Listener::Listener(
                boost::asio::io_context &ioc, const tcp::endpoint &endpoint
            )
                : acceptor_(ioc)
                , socket_(ioc) {
                error_code ec;
            ...
            void Listener::do_accept() {
                acceptor_.async_accept(
                    socket_, [self = shared_from_this()](const error_code &ec) {
                        self->on_accept(ec);
                    }
                );
            }
