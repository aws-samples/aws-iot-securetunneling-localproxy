...
const int IO_PAUSE_MS = 50;
const size_t READ_BUFFER_SIZE = 63 * 1024;
const char *const LOCALHOST = "127.0.0.1";

/**
 * Check if error code indicates socket was closed by peer.
 * Different platforms return different error codes:
 * - Windows: connection_reset (WSAECONNRESET) or connection_aborted
 * (WSAECONNABORTED)
 * - Unix/Linux: no_such_file_or_directory
 */
#ifdef _WIN32
auto const EC_CONNECTION_RESET = boost::asio::error::connection_reset;
const auto EC_CONNECTION_ABORTED = boost::asio::error::connection_aborted;

bool is_socket_closed_error(const boost::system::error_code &ec) {
    ...
    namespace securedtunneling {
        namespace test {
            void apply_test_config(
                LocalproxyConfig &cfg, const tcp::endpoint &ws_endpoint
            ) {
...
namespace iot {
    namespace securedtunneling {
        namespace test {
            namespace config {
                const std::string DEFAULT_CONFIG_DIR_SUFFIX = "/config";

                /**
                 * Note: catch2 does not support mocking. So for now, create the
                 * files and directories. For future, we can integrate with
                ...
                TEST_CASE("Unit tests for ConfigFile", "[config]") {
                    ...
                    if (fs::exists(test_dir)) {
                        try {
                            ...
                            std::cout << "Error deleting test dir " << e.what()
                                      << std::endl;
                        }
                    }
                    REQUIRE(true == fs::create_directory(test_dir));

                    const string test_config_file_name = "configFile";
                    bool ok = static_cast<bool>(
                        std::ofstream(test_config_file_name)
                    );
                    REQUIRE(ok == true);
                    /**
                    ...
                    SECTION("Test is_valid_directory: valid directory") {
                        CHECK(
                            true
                            == aws::iot::securedtunneling::config_file::
                    ...
                    SECTION("Test is_valid_directory: empty directory") {
                        CHECK(
                            false
                            == aws::iot::securedtunneling::config_file::
                                is_valid_directory(test_dir)
                        );
                    }

                    SECTION(
                        "Test is_valid_directory: pass a file instead of a "
                        "directory"
                    ) {
                        CHECK(
                            false
                            == aws::iot::securedtunneling::config_file::
                                is_valid_directory(test_config_file_name)
                        );
                    }
                    ...
                    SECTION("Test happy path for get_all_files") {
                        CHECK_NOTHROW(
                            aws::iot::securedtunneling::config_file::
                                get_all_files(current_dir)
                        );
                    }

                    SECTION(
                        "Test happy path for "
                        "read_service_ids_from_config_files, 1 service id"
                    ) {
                        std::unordered_map<string, string>
                            serviceId_to_endpoint_mapping {};
                        ...
                        CHECK(
                            serviceId_to_endpoint_mapping[identifier]
                            == endpoint
                        );
                    }
                    ...
                    SECTION(
                        "Test happy path for "
                        "read_service_ids_from_config_files, 0 service id"
                    ) {
                        std::unordered_map<string, string>
                            serviceId_to_endpoint_mapping {};
                        std::vector<std::string> file_paths {};
                    ...
                    SECTION("Test happy path for find_service_ids") {
                        ...
                        aws::iot::securedtunneling::config_file::
                            update_port_mapping(
                                file_content, serviceId_to_endpoint_mapping
                            );
                        CHECK(serviceId_to_endpoint_mapping.size() == 1);
                        CHECK(
                            serviceId_to_endpoint_mapping[identifier]
                    ...
                    SECTION(
                        "Test happy path for get_default_port_mapping_dir"
                    ) {
                        CHECK(
                            aws::iot::securedtunneling::config_file::
                                get_default_port_mapping_dir()
                            == current_dir + DEFAULT_CONFIG_DIR_SUFFIX
                        );
                    }

                    // Test case clean up.
                    int remove_file_stat
                        = std::remove(test_config_file_name.c_str());
                    if (remove_file_stat != 0) {
                        std::cout << "Error deleting file "
                                  << test_config_file_name << std::endl;
                    }
                    // Can comment out below line if does not want to check for
                    // clean up.
...
TEST_CASE("Test source mode", "[source]") {
     ...
     * 2. Create web socket server to act as secure tunneling service (cloud
     * side).
    ...
    std::cout << "Test server is listening on address: " << ws_address.address()
              << " and port: " << ws_address.port() << endl;
    ...
    adapter_cfg.serviceId_to_endpoint_map[service_id]
        = boost::lexical_cast<std::string>(adapter_chosen_port);
    ...
    CHECK(
        ws_server.get_handshake_request().method()
        == boost::beast::http::verb::get
    );
    CHECK(
        ws_server.get_handshake_request().target()
        == "/tunnel?local-proxy-mode=source"
    );
    CHECK(
        ws_server.get_handshake_request().base()["sec-websocket-protocol"]
        == "aws.iot.securetunneling-3.0"
    );
    CHECK(
        ws_server.get_handshake_request().base()["access-token"]
        == adapter_cfg.access_token
    );
    ...
    client_socket.connect(
        tcp::endpoint {
            boost::asio::ip::make_address(adapter_cfg.bind_address.get()),
            adapter_chosen_port }
    );
    ...
    for (int i = 0; i < 5; ++i) {
        const string test_string
            = (boost::format("test message: %1%") % i).str();
        client_socket.send(boost::asio::buffer(test_string));
        client_socket.read_some(
            boost::asio::buffer(
                reinterpret_cast<void *>(read_buffer), READ_BUFFER_SIZE
            )
        );
    ...
    ws_server.expect_next_message([](const message &msg) {
        return (msg.type()
                == com::amazonaws::iot::securedtunneling::
                    Message_Type_CONNECTION_RESET)
            && msg.streamid() == 1;
    });
    client_socket.close();
    ...
    ws_server.close_client(
        "test_closure", boost::beast::websocket::internal_error
    );
    // attempt a read on the client which should now see the socket EOF (peer
    // closed) caused by adapter
    client_socket.read_some(
        boost::asio::buffer(
            reinterpret_cast<void *>(read_buffer), READ_BUFFER_SIZE
        ),
...
TEST_CASE("Test source mode with client token", "[source]") {
    ...
    CHECK(
        ws_server.get_handshake_request().base()["client-token"]
        == adapter_cfg.client_token
    );
...
TEST_CASE("Test destination mode", "[destination]") {
    ...
    tcp::acceptor acceptor { io_ctx,
                             { boost::asio::ip::make_address(LOCALHOST), 0 } };
    cout << "Destination app listening on address: "
    ...
    // start web socket server thread and tcp adapter threads
    thread ws_server_thread { [&ws_server]() { ws_server.run(); } };
    std::cout << "Test server listening on address: " << ws_address.address()
              << " and port: " << ws_address.port() << endl;
    ...
    CHECK(
        ws_server.get_handshake_request().target()
        == "/tunnel?local-proxy-mode=destination"
    );
    CHECK(
        ws_server.get_handshake_request().base()["sec-websocket-protocol"]
        == "aws.iot.securetunneling-3.0"
    ...
    ws_server.close_client(
        "test_closure", boost::beast::websocket::internal_error
    ); // need to perform write to trigger close
    // attempt a read on the client which should now see the socket EOF (peer
    // closed) caused by adapter
    destination_socket.read_some(
        boost::asio::buffer(
