...
- Install the following dependencies (Choose visual studio command prompt based
  on architecture):
  - Download and install zlib:
    ...
    - Update PATH environment variable to add the dll for zlib which is inside
  ...
  - Download and install openssl
    ...
    - `perl Configure { VC-WIN32 | VC-WIN64A | VC-WIN64I | VC-CE }` (Choose one
      of the options based on your architecture, usually WIN64A if you are on
      the x86_64 platform).
    - `nmake`
    - `nmake install` (installs OpenSSL inside C:\Program Files\)
    - Update PATH environment variable to add the dll for openssl which is
      inside C:\Program Files\OpenSSL\bin
    - Note: In some x86_64 windows configurations, compiling for a 64 bit target
      may fail. You may instead install openssl through Chocolatey. The
      installation folder name might be OpenSSL-Win64. The project build command
      below should be changed correspondingly.
  ...
  - Download and install catch2
    ...
    - `cmake -DBUILD_TESTING=OFF -G "NMake Makefiles" -DCMAKE_BUILD_TYPE=Release ../`
      ( Install python if you want to execute test )
    - `nmake`
  ...
  - Download and install boost
    ...
    - `bootstrap.bat`
    - `.\b2 toolset=msvc address-model={32 | 64} install
      define=BOOST_USE_WINAPI_VERSION=\<hex value> . ( installs boost inside
      C:\)
      - Replace \<hex value> with the appropriate macro from
        [here](https://learn.microsoft.com/en-us/windows/win32/winprog/using-the-windows-headers?redirectedfrom=MSDN#macros-for-conditional-declarations)
  ...
  - Download and build aws-iot-securetunneling-localproxy
    ...
    - `mkdir build`
    - `cd build`
    - Build the cmake project. Replace \<hex value> with the appropriate value
      based on
    ...
    - `msbuild localproxy.vcxproj -p:Configuration=Release` ( builds
      localproxy.exe inside bin\Release folder )

  - Follow
    [instructions](https://github.com/aws-samples/aws-iot-securetunneling-localproxy)
    under heading `Security Considerations` to run local proxy on a window OS.
