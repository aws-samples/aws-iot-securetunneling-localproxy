...
for ((i = 0; i < ${#OUTPUT_LIST[@]}; i++)); do
  ...
  if [ ${#PACKAGE_DETAILS[@]} -eq "3" ]; then
    ...
    PACKAGE_LOCATION=""
    for ((license_file_dir = 0; license_file_dir < "${#LICENSE_FILE_LOCATIONS[@]}"; license_file_dir++)); do
      for ((license_file = 0; license_file < "${#LICENSE_FILE_NAMES[@]}"; license_file++)); do
