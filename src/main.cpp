...
const char *const ACCESS_TOKEN_ENV_VARIABLE = "AWSIOT_TUNNEL_ACCESS_TOKEN";
const char *const CLIENT_TOKEN_ENV_VARIABLE = "AWSIOT_TUNNEL_CLIENT_TOKEN";
const char *const ENDPOINT_ENV_VARIABLE = "AWSIOT_TUNNEL_ENDPOINT";
const char *const REGION_ENV_VARIABLE = "AWSIOT_TUNNEL_REGION";
...
void log_formatter(
    boost::log::formatting_ostream &strm, const boost::log::record_view &rec
) {
    // Example log format:
    //[2019-01-17T21:37:15.528290]{632}[trace]   Waiting for stream start...
    std::ostringstream severity;
    // severity needs to be pulled out of the inline stream operations due to a
    // gcc-8.2.0 compile error (seemingly a bug?) error: 'class
    // std::basic_ostream<char>' has no member named 'str'    -- clang and msvc
    // are fine .str() is not a member of std::basic_ostream<char> but is a
    // member of ostringstream
    severity << "["
             << boost::log::trivial::to_string(
                    rec["Severity"]
    ...
    strm << "["
         << boost::posix_time::to_iso_extended_string(
                rec["TimeStamp"].extract<boost::posix_time::ptime>().get()
            )
         << "]" << "{" << std::dec
         << rec["ProcessID"]
                .extract<
...
bool process_cli(
    ...
    if (vm.count("help")) {
        ...
        boost::property_tree::json_parser::write_json(
            export_path, settings, std::locale(), true
        );
        return false;
    }
    ...
    // dont allow above settings to be impacted by configuration file or
    ...
    store(
        parse_environment(
            ...
            [](std::string name) -> std::string {
                if (name == ACCESS_TOKEN_ENV_VARIABLE) return "access-token";
                if (name == CLIENT_TOKEN_ENV_VARIABLE) return "client-token";
                if (name == ENDPOINT_ENV_VARIABLE) return "proxy-endpoint";
                if (name == REGION_ENV_VARIABLE) return "region";
                return "";
            }
        ),
        vm
    ...
    if (vm.count("settings-json")) {
        ...
        BOOST_LOG_TRIVIAL(info)
            << "Using settings specified in file: " << settings_path;
    ...
    if (token_cli_warning) {
        BOOST_LOG_TRIVIAL(warning)
            ...
            << ACCESS_TOKEN_ENV_VARIABLE << " instead";
    }
    ...
    string proxy_endpoint = vm.count("proxy-endpoint") == 1
        ? vm["proxy-endpoint"].as<string>()
        : get_region_endpoint(vm["region"].as<string>(), settings);

    transform(
        proxy_endpoint.begin(),
        proxy_endpoint.end(),
        proxy_endpoint.begin(),
        ::tolower
    );
    tuple<string, uint16_t> proxy_host_and_port = get_host_and_port(
        proxy_endpoint, aws::iot::securedtunneling::DEFAULT_PROXY_SERVER_PORT
    );
    cfg.proxy_host = std::get<0>(proxy_host_and_port);
    cfg.proxy_port = std::get<1>(proxy_host_and_port);

    // https_proxy environment variable takes precedence over HTTPS_PROXY
    // environment variable
    const char *lowercase_web_proxy_endpoint
        = std::getenv(web_proxy_env_variable);
    const char *upper_web_proxy_endpoint = std::getenv(WEB_PROXY_ENV_VARIABLE);
    const string web_proxy_endpoint = lowercase_web_proxy_endpoint != nullptr
        ? std::string(lowercase_web_proxy_endpoint)
        : upper_web_proxy_endpoint != nullptr
        ? std::string(upper_web_proxy_endpoint)
        : "";
    if (!web_proxy_endpoint.empty()) {
        std::shared_ptr<aws::iot::securedtunneling::url> url = nullptr;
        try {
            url = std::make_shared<aws::iot::securedtunneling::url>(
                web_proxy_endpoint
            );
        } catch (exception &e) {
            BOOST_LOG_TRIVIAL(fatal)
                << "Failed to parse the value of environment variable "
                << (lowercase_web_proxy_endpoint != nullptr
                        ? web_proxy_env_variable
                        : WEB_PROXY_ENV_VARIABLE);
            throw e;
        }
        ...
        if (url->port == 0) {
            cfg.web_proxy_port
                = aws::iot::securedtunneling::DEFAULT_WEB_PROXY_SERVER_PORT;
            BOOST_LOG_TRIVIAL(warning)
                << "No port was was provided for the web proxy, using default: "
                << cfg.web_proxy_port;
        } else {
        ...
        BOOST_LOG_TRIVIAL(info)
            << "Found Web proxy information in the environment variables, will "
               "use it to connect via the proxy.";
    }

    cfg.mode = vm.count("destination-app") == 1 ? proxy_mode::DESTINATION
                                                : proxy_mode::SOURCE;
    ...
    if (vm.count("destination-client-type")) {
        ...
        if (type == "V1") {
            ...
            BOOST_LOG_TRIVIAL(warning)
                << "unknown value for destination-client-type, assuming "
                   "default protocol V3.";
        }
    }
     ...
     * 4. At least one of the parameter should be provided to start local proxy
    ...
    if (vm.count("destination-app")) {
        cfg.mode = proxy_mode::DESTINATION;
        update_port_mapping(
            vm["destination-app"].as<string>(), cfg.serviceId_to_endpoint_map
        );
        // Support v1 local proxy format
        if (cfg.serviceId_to_endpoint_map.size() == 1
            && cfg.serviceId_to_endpoint_map.begin()->first.empty()) {
            BOOST_LOG_TRIVIAL(debug)
                << "v2 local proxy starts with v1 local proxy format";
        } else {
            BOOST_LOG_TRIVIAL(debug)
                << "Detect port mapping configuration provided through CLI in "
                   "destination mode:";
            BOOST_LOG_TRIVIAL(debug)
                << "----------------------------------------------------------";
    ...
    if (vm.count("source-listen-port")) {
        cfg.mode = proxy_mode::SOURCE;
        update_port_mapping(
            vm["source-listen-port"].as<string>(), cfg.serviceId_to_endpoint_map
        );
        // Support v1 local proxy format
        if (cfg.serviceId_to_endpoint_map.size() == 1
            && cfg.serviceId_to_endpoint_map.begin()->first.empty()) {
            ...
            BOOST_LOG_TRIVIAL(debug) << "Detect port mapping configuration "
                                        "provided through CLI in source mode:";
            BOOST_LOG_TRIVIAL(debug)
                << "----------------------------------------------------------";
            for (auto m : cfg.serviceId_to_endpoint_map) {
                BOOST_LOG_TRIVIAL(debug) << m.first << " = " << m.second;
    ...
    if (vm.count("config-dir")) {
        ...
        BOOST_LOG_TRIVIAL(debug) << "Detect port mapping configuration "
                                    "provided through configuration directory :"
                                 << config_dir;
        // Run validation against the input
        if (!is_valid_directory(config_dir)) {
            std::string error_message
                = std::string("Invalid configuration directory: ") + config_dir;
    ...
    if (cfg.mode == proxy_mode::SOURCE && cfg.config_files.empty()
        && cfg.serviceId_to_endpoint_map.empty()) {
        BOOST_LOG_TRIVIAL(debug)
            << "Local proxy does not detect any port mapping configuration. "
               "Will pick up random ports to run in source mode.";
    }
    ...
    if (cfg.no_ssl_host_verify) {
        BOOST_LOG_TRIVIAL(warning) << "****************************************"
                                      "******************************";
        BOOST_LOG_TRIVIAL(warning)
            << "* SECURITY WARNING: SSL host verification is DISABLED (-k "
               "flag)     *";
        BOOST_LOG_TRIVIAL(warning) << "* This removes critical security checks "
                                      "and exposes the connection  *";
        BOOST_LOG_TRIVIAL(warning) << "* to potential man-in-the-middle "
                                      "attacks. DO NOT use in production! *";
        BOOST_LOG_TRIVIAL(warning) << "****************************************"
                                      "******************************";
    }
...
int main(int argc, char **argv) {
    try {
        ...
        std::cout << "Running AWS IoT Secure Tunneling Local Proxy version: "
                  << version << std::endl;
