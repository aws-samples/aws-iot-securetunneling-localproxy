    ...
    namespace securedtunneling {
        namespace connection {
            ...
            public:
                ...
                /**
                 * A buffer holding data that will be sent to secure tunneling
                 * server through web socket connection. This buffer will only
                 * hold data belongs to its own stream in a multiplexed tunnel.
                 */
                boost::beast::flat_buffer outgoing_message_buffer_;
                // Buffer sequence storing the raw bytes read from the tcp
                // socket reads to send over web socket. The bytes in this
                // buffer represent the raw application data not already
                // packaged in protobuf messages. This allows us to condense
                // smaller TCP read chunks to bigger web socket writes. It also
                // makes it impossible to "inject" a non-data message in data
                // sequence order
                boost::beast::multi_buffer web_socket_data_write_buffer_;

                uint32_t
                    connection_id_; // assigned connection_id for tcp connection
                ...
                // function object defines what to do upon receiving control
                // message
                std::function<bool(const message &)> on_control_message
                    = nullptr;
                // function object defines what to do upon receiving data
                // message
                std::function<bool(const message &)> on_data_message = nullptr;
                // function object defines what to do if there is a tcp error
                // occurred
                std::function<void(const boost::system::error_code &)>
                    on_tcp_error = nullptr;
                // function object defines what to do when tcp_write_buffer_
                // drain has completed
                std::function<void()> on_tcp_write_buffer_drain_complete
                    = nullptr;
                // function object defines what to do when
