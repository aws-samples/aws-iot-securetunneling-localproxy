...
namespace iot {
    namespace securedtunneling {
        ...
        WebProxyAdapter::WebProxyAdapter(
            logger *log, const LocalproxyConfig &localproxy_config
        )
            : log(log)
            , localproxy_config(localproxy_config) {
        }

        void WebProxyAdapter::async_connect(
            BoostCallbackFunc on_tcp_tunnel_callback,
            const shared_ptr<WebSocketStream> &wss,
            const tcp::endpoint &web_proxy_endpoint
        ) {
            on_tcp_tunnel = std::make_unique<BoostCallbackFunc>(
                std::move(on_tcp_tunnel_callback)
            );
            websocket_stream = wss.get();
            BOOST_LOG_SEV(*log, trace)
                << "Establishing TCP connection with the Web Proxy";
            websocket_stream->get_tcp_socket().async_connect(
                web_proxy_endpoint, [this](const error_code &ec) {
                    if (ec) {
                        BOOST_LOG_SEV(*log, error)
                            << (boost::format(
                                    "Could not connect to Web Proxy: %1%"
                                )
                                % ec.message())
                                   .str();
                        ...
                        websocket_stream->lowest_layer().set_option(
                            tcp::no_delay(true)
                        );
        ...
        void WebProxyAdapter::async_ssl_handshake() {
            if (!localproxy_config.no_ssl_host_verify) {
                websocket_stream->get_web_proxy_ssl_stream()->set_verify_mode(
                    ssl::verify_peer | ssl::verify_fail_if_no_peer_cert
                );
                websocket_stream->get_web_proxy_ssl_stream()
                    ->set_verify_callback(
                        boost::asio::ssl::host_name_verification(
                            localproxy_config.web_proxy_host
                        )
                    );
            } else {
                BOOST_LOG_SEV(
                    *log, warning
                ) << "SECURITY WARNING: SSL host verification is disabled for "
                     "web proxy - connection is vulnerable to MITM attacks";
            }
            websocket_stream->get_web_proxy_ssl_stream()->async_handshake(
                ssl::stream_base::client, [this](const error_code &ec) {
                    if (ec) {
                        BOOST_LOG_SEV(*log, error)
                            << (boost::format(
                                    "Could not perform SSL handshake with Web "
                                    "Proxy: %1%"
                                )
                                % ec.message())
                                   .str();
                        (*on_tcp_tunnel)(
                            WebProxyAdapterErrc::SslHandshakeError
                        );
                    } else {
                        BOOST_LOG_SEV(*log, debug)
                            << "Performed SSL handshake with Web proxy "
                               "successfully";
                        on_tcp_connect();
                    }
                }
        ...
        void WebProxyAdapter::on_tcp_connect() {
            ...
            if (!localproxy_config.web_proxy_auth.empty()) {
                BOOST_LOG_SEV(*log, trace)
                    << "Web proxy AuthN found, adding them to the request";
                ...
                request.set(
                    http::field::proxy_authorization, "basic " + credentials
                );
            }
            ...
            auto on_async_write =
                [this](const error_code &ec, std::size_t bytes_transferred) {
                    ...
                    if (ec) {
                        BOOST_LOG_SEV(*log, error)
                            << (boost::format(
                                    "Could not send HTTP CONNECT request to "
                                    "the Web proxy: %1%"
                                )
                                % ec.message())
                                   .str();
                        (*on_tcp_tunnel)(
                            WebProxyAdapterErrc::HttpWriteRequestError
                        );
                    } else {
                        BOOST_LOG_SEV(
                            *log, debug
            ...
            if (localproxy_config.is_web_proxy_using_tls) {
                ...
                http::async_write(
                    websocket_stream->get_tcp_socket(), request, on_async_write
                );
            }
        }
        ...
        void WebProxyAdapter::on_http_connect_write() {
            ...
            auto on_read = [this, response_buffer](
                ...
                if (ec) {
                    BOOST_LOG_SEV(*log, error)
                        << (boost::format(
                                "Could not read HTTP CONNECT response from the "
                                "Web proxy: %1%"
                            )
                            % ec.message())
                               .str();
                ...
                case http::status_class::successful:
                    if (response.result() == http::status::ok) {
                        ...
                        BOOST_LOG_SEV(*log, warning)
                            << "TCP tunnel established but with unexpected "
                               "response code from the Web proxy";
                    }
                ...
                case http::status_class::redirection:
                    BOOST_LOG_SEV(
                        *log, error
                    ) << "Make sure you're using the correct Web proxy address";
                    (*on_tcp_tunnel)(WebProxyAdapterErrc::RedirectionError);
                    break;
                case http::status_class::client_error:
                    BOOST_LOG_SEV(*log, error)
                        << "Make sure the Web proxy is configured properly";
                    (*on_tcp_tunnel)(WebProxyAdapterErrc::ClientError);
                    break;
                case http::status_class::server_error:
                    BOOST_LOG_SEV(*log, error) << "Web proxy error, make sure "
                                                  "to check your server's logs";
                    (*on_tcp_tunnel)(WebProxyAdapterErrc::ServerError);
                    break;
                default:
            ...
            // TCP connection causing "stream truncated" error during the SSL
            // handshake with the proxy server. So I had to read from the TCP
            // socket directly and parse the response. This could be something
            // to test when we upgrade to a newer version of boost to see if
            // boost::beast::http::async_read is fixed in it or not.
            if (localproxy_config.is_web_proxy_using_tls) {
                websocket_stream->get_web_proxy_ssl_stream()->async_read_some(
                    read_buffer, on_read
                );
            } else {
                websocket_stream->get_tcp_socket().async_receive(
                    read_buffer, on_read
                );
            }
        }
