        ...
        namespace config_file {
            ...
            bool is_valid_directory(const string &file_dir) {
                ...
                try {
                     ...
                     */
                    if (filesys::exists(path_obj)
                        && filesys::is_directory(path_obj)
                        && (!filesys::is_empty(path_obj))) {
                        is_dir = true;
                        ...
                        BOOST_LOG_SEV(log, debug)
                            << file_dir
                            << " empty dir! Please add configuration files.";
                    else
                        BOOST_LOG_SEV(log, debug)
                            << file_dir << " is not valid!";
             ...
             * @param file_dir : directory file path
             * @return file paths under the given directory and its
             * subdirectories.
             */
            vector<string> get_all_files(const string &file_dir) {
                ...
                for (filesys::recursive_directory_iterator dir_itr(file_dir);
                     dir_itr != end_iter;
                     ++dir_itr) {
                    ...
                    if (filesys::is_regular_file(dir_itr->status())) {
                        ...
                        files_under_directory.push_back(
                            dir_itr->path().generic_string()
                        );
                    }
                }
            ...
            void read_service_ids_from_config_files(
                ...
                for (auto file_path : file_paths) {
                    ...
                    if (serviceId_to_endpoint_mapping.size()
                        == service_ids.size()) {
                        break;
                    }
                    // Parse file in .ini format, if having issues, skip this
                    // file and read the next file in the folder.
                    try {
                        ...
                        BOOST_LOG_SEV(
                          ...
                          << file_path
                          << " .Please make sure your file is in .ini format.";
                        BOOST_LOG_SEV(log, warning)
                            << "Error message from parsing: " << e.what()
                            << " .Continue to the next file.";
                        continue;
                    }
                    for (auto service_id : service_ids) {
                        /**
                         * Search for service ids that does not have a port
                         * mapping detected. If more than one service id
                         * mappings found in the configuration files, use the
                         * first one found.
                         */
                        if (serviceId_to_endpoint_mapping.find(service_id)
                            != serviceId_to_endpoint_mapping.end()) {
                            continue;
                        }
                        try {
                            string endpoint = pt.get<std::string>(service_id);
                            serviceId_to_endpoint_mapping.insert(
                                { service_id, endpoint }
                            );
                        } catch (boost::property_tree::ptree_bad_path &e) {
                            BOOST_LOG_SEV(log, warning)
                                << "Fail to read file: " << file_path
            ...
            /**
             * Interpret the CLI mappings for -s and -d and use this information
             * to build: service_id to endpoint(address:port or port) mapping
             * @param cli_input: the string from -s and -d in the CLI. Example:
             * -s SSH1=5555,SSH2=6666
             * @param serviceId_to_endpoint_mapping: the mapping to be updated:
             * service_id -> endpoint Mapping update is in place.
             */
            void update_port_mapping(
                ...
                if (splitting_1st_res.empty()) {
                    throw std::runtime_error(
                        "Must provide at least one port or port mapping for "
                        "destination-app!"
                    );
                }
                ...
                for (auto res : splitting_1st_res) {
                    ...
                    // Inside the mapping, the service_id and port are delimited
                    // by =
                    boost::split(
                        splitting_2rd_res,
                        res,
                    ...
                    if (splitting_2rd_res.size() != 2) {
                        /** For v1 format, v2 local proxy will continue to
                         * support Example 1: Local proxy starts in v1 source
                         * mode:
                         *  ./localproxy -r us-east-1 -s 3389 -t
                         * <source_client_access_token> cli_input will be 3389
                         *  Example 2: Local proxy starts in v1 destination
                         * mode:
                         *  ./localproxy -r us-east-1 -d localhost:22 -t
                         * <destination_client_access_token> cli_input will be
                         * localhost:22
                         */
                        if (splitting_1st_res.size() == 1
                            && splitting_2rd_res.size() == 1) {
                            boost::trim(splitting_2rd_res[0]);
                            input_validation::validate_destination_endpoint(
                                splitting_2rd_res[0]
                            );
                            ...
                            throw std::runtime_error(
                                "Wrong format for the port mappings! Example: "
                                "SSH1=5555,SSH2=6666."
                            );
                        }
                    }
                    ...
                    if (service_id.empty() || endpoint.empty()) {
                        string error_message
                            = string("Wrong format for the port mappings: ")
                            + res + string(" .Example: SSH1=5555");
                    ...
                    // Check if it's a duplicate mapping, ignore if it has been
                    // provided
                    if (serviceId_to_endpoint_mapping.find(service_id)
                        != serviceId_to_endpoint_mapping.end()) {
                        BOOST_LOG_SEV(log, warning)
                            << "Duplicate mappings, ignore. This mapping "
                               "already exists: "
                            << service_id << " : "
