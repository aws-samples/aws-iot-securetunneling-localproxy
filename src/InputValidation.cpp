...
{
    BOOST_LOG_TRIVIAL(warning) << "SECURITY: Rejected input for '" << input_name << "': " << reason;
}
...
void validate_access_token(const std::string& token)
{
    ...
    if (token.length() > MAX_ACCESS_TOKEN_LENGTH) {
        log_rejected_input("access-token", "exceeds maximum length");
        throw std::runtime_error("Access token exceeds maximum allowed length");
    }
    // Reject control characters and null bytes
    for (char c : token) {
        if (c < 0x20 || c == 0x7F) {
            log_rejected_input("access-token", "contains control characters");
            throw std::runtime_error("Access token contains invalid control characters");
        }
    }
}
...
{
    if (token.empty()) {
        BOOST_LOG_TRIVIAL(trace) << "Client token is empty, skipping validation";
        return; // Client token is optional
    }
    if (token.length() < MIN_CLIENT_TOKEN_LENGTH || token.length() > MAX_CLIENT_TOKEN_LENGTH) {
        log_rejected_input("client-token", "invalid length (must be 32-128 chars)");
        throw std::runtime_error("Client token must be between 32 and 128 characters");
    }
    // Reject control characters and null bytes
    for (char c : token) {
        if (c < 0x20 || c == 0x7F) {
            log_rejected_input("client-token", "contains control characters");
            throw std::runtime_error("Client token contains invalid control characters");
        }
    }
}
...
void validate_endpoint(const std::string& endpoint)
{
    ...
    if (endpoint.length() > MAX_ENDPOINT_LENGTH) {
        log_rejected_input("proxy-endpoint", "exceeds maximum length");
        throw std::runtime_error("Proxy endpoint exceeds maximum allowed length");
    }
    // Valid hostname/IP with optional port: hostname:port or hostname
    // Allow alphanumeric, dots, hyphens, colons (for port), and brackets (for IPv6)
    static const std::regex endpoint_pattern("^[a-zA-Z0-9.:\\[\\]-]+$");
    if (!std::regex_match(endpoint, endpoint_pattern)) {
        log_rejected_input("proxy-endpoint", "invalid characters");
        throw std::runtime_error("Proxy endpoint contains invalid characters");
    }
}
...
void validate_region(const std::string& region)
{
    ...
    if (region.length() > MAX_REGION_LENGTH) {
        ...
        throw std::runtime_error("Region exceeds maximum allowed length");
    }
    ...
    if (!std::regex_match(region, region_pattern)) {
        ...
        throw std::runtime_error("Region must contain only lowercase letters, numbers, and hyphens");
    }
}
...
void validate_service_id(const std::string& service_id)
{
    ...
    if (service_id.length() > MAX_SERVICE_ID_LENGTH) {
        ...
        throw std::runtime_error("Service ID exceeds maximum allowed length");
    }
    ...
    if (!std::regex_match(service_id, service_id_pattern)) {
        ...
        throw std::runtime_error("Service ID must contain only alphanumeric characters, hyphens, and underscores");
    }
}
...
void validate_path(const std::string& path)
{
    ...
    if (path.length() > MAX_PATH_LENGTH) {
        ...
        throw std::runtime_error("Path exceeds maximum allowed length");
    }
...
void validate_bind_address(const std::string& address)
{
    ...
    if (address.length() > MAX_BIND_ADDRESS_LENGTH) {
        log_rejected_input("bind-address", "exceeds maximum length");
        throw std::runtime_error("Bind address exceeds maximum allowed length");
    }
    // Valid hostname/IP: alphanumeric, dots, hyphens, colons (IPv6), brackets
    static const std::regex address_pattern("^[a-zA-Z0-9.:\\[\\]-]+$");
    if (!std::regex_match(address, address_pattern)) {
        log_rejected_input("bind-address", "invalid characters");
        throw std::runtime_error("Bind address contains invalid characters");
    }
}
...
void validate_destination_endpoint(const std::string& endpoint)
{
    ...
    // Extract and validate port if present (last colon not inside brackets)
    size_t last_colon = endpoint.rfind(':');
    if (last_colon != std::string::npos) {
        // Check it's not inside IPv6 brackets
        size_t bracket_close = endpoint.rfind(']');
        if (bracket_close == std::string::npos || last_colon > bracket_close) {
