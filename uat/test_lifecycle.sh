...
[[ -z "$TUNNEL_ID" || "$TUNNEL_ID" == "null" ]] && { log_error "Open tunnel failed"; exit 1; }
...
STATUS=$(aws iotsecuretunneling describe-tunnel \
    ...
    --query 'tunnel.status' \
    --output text)
[[ "$STATUS" == "OPEN" ]] || { log_error "Describe tunnel failed: $STATUS"; exit 1; }
...
LIST_OUTPUT=$(aws iotsecuretunneling list-tunnels \
    ...
    --output json)
[[ $(echo "$LIST_OUTPUT" | jq 'length') -gt 0 ]] || { log_error "List tunnels failed"; exit 1; }
